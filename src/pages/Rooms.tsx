...
const Rooms = () => {
  ...
  const handleCheckoutRoom = async (room: Room) => {
    ...
    try {
      ...
      // Keep 0 for complimentary instead of falling back to room price
      const baseAmount = (guest.totalAmount ?? room.price)
      
      setCheckoutRoom(room)
      // Use ISO date (yyyy-mm-dd) for HTML date input compatibility
  ...
  const generateBill = async () => {
    ...
    try {
      ...
      const { formattedArrivalDate, formattedDepartureDate } = formatBillDates(guest.checkInDate, checkoutDetails.actualCheckOutDate)
          ...
          <div class="header">
            ...
            <div class="hotel-details">
              ...
              <div class="stay-details">
                ...
                <div class="info-row editable" contenteditable="false">Payment Method: ${((checkoutDetails.paymentMethod || guest.paymentMethod || 'CASH')).toUpperCase()}</div>
              </div>
            </div>
                      ...
                      onClick={async () => {
                        ...
                        await (async function previewBill() {
                          ...
                          try {
                            ...
                            const checkInTime = new Date(guest.createdAt || guest.updatedAt || Date.now())
                            const formattedCheckInTime = checkInTime.toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' })
                            // Robust date parsing for both yyyy-mm-dd and dd-mm-yyyy
                            const parseFlex = (str: string) => {
                              try {
                            ...
                            const checkInDate = parseFlex(guest.checkInDate)
                            const checkOutDate = parseFlex(checkoutDetails.actualCheckOutDate)
                            let daysDiff = 1
                            ...
                            // Complimentary: waive room rent and extra bed only (add-ons remain billable)
                            const isComplimentary = (guest.complimentary === true) || (String(guest.complimentary).toLowerCase() === 'true')
                            if (isComplimentary) { pricePerDay = 0; extraBedCharges = 0 }
                            ...
                            const halfDayTaxableValue = halfDay > 0 ? halfDay / 1.05 : 0
                            ...
                            // Display-safe values for complimentary stays
