  ...
  const handleCheckoutRoom = async (room: Room) => {
    ...
    try {
      ...
      setCheckoutDetails({
        ...
        halfDayCharges: 0,
        paymentMethod: 'CASH',
        payments: [{ method: 'CASH', amount: baseAmount }],
        notes: '',
                    ...
                    <div className="mt-2 space-y-1">
                      {checkoutDetails.payments.map((p, idx) => (
                        <div key={idx} className="flex items-center text-sm justify-between">
                          ...
                          <input type="number" className="input-field ml-2 w-28" value={p.amount} min={0}
                            onChange={(e) => {
                          ...
                          />
                          <button type="button" className="ml-2 text-xs text-red-600"
                            onClick={() => {
                      ...
                      onClick={async () => {
                        ...
                        await (async function previewBill() {
                          ...
                          try {
                            ...
                            const checkInDate = new Date(parseInt(checkInParts[0]), parseInt(checkInParts[1]) - 1, parseInt(checkInParts[2]))
                            const coStr = checkoutDetails.actualCheckOutDate || (function(){ const t=new Date(); const dd=String(t.getDate()).padStart(2,'0'); const mm=String(t.getMonth()+1).padStart(2,'0'); const yyyy=t.getFullYear(); return `${dd}-${mm}-${yyyy}` })()
                            const outParts = coStr.split('-')
