...
const DeleteRooms = () => {
  ...
  // Load current rooms from backend
  useEffect(() => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    // Optional password field in UI; skip hardcoded validation
    if (!password.trim()) {
    ...
    try {
      // Delete on backend
      const res = await fetch(`${BACKEND_URL}/api/rooms/${selectedRoom.id}`, { method: 'DELETE' })
